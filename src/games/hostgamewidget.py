...
class HostgameWidget(FormClass, BaseClass):
    ...
    def hosting(self):
        ...
        if not self.iscoop or True:
            ...
            self.parent.client.send(dict(command="game_host",
                                  access="password" if self.password else "public",
                                  password=self.password,
                                  mod=self.featured_mod,
                                  title=self.title,
                                  mapname=self.mapname,
                                  gameport=self.parent.client.gamePort))
